...
Given(
  'I have exported an EZDrummer 2 track from my DAW, as MIDI',
  () => {
...
When(
  'I ask BamBam to remap that track to General MIDI Percussion',
  () => {
...
Then(
  'the re-mapped track should be a valid MIDI track that others can read',
  () => {
    return 'pending';
  },
);
...
Then(
  ...
  () => {
    ...
    expect(
      gmTrack.noteNumbersAt({ measure: 1, beat: 1, tick: 0 }),
    ).to.eql([42]);
  },
);
